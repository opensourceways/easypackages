        ...
        for repos_info in self.repo_info:
            ...
            if self.strategy == "all":
                ...
                command = "submit -m -c translate_job.yaml rpm_baseurl={0} \
                        repo_name={1} translate_strategy=all".format(baseurl, channel)
                subprocess.Popen(command, shell=True, text=True, capture_output=True)
                ...
                if repos_info["repo_name"] == "update":
                    if repos_info["watch_update"]:
                        command = "submit -m -c translate_job.yaml rpm_baseurl={0} \
                                repo_name={1} translate_strategy=delta".format(baseurl, channel)
                        subprocess.Popen(command, shell=True, text=True, capture_output=True)
            count += 1
            submit_job_count += 1
            sleep(30)
            if count >= 5:
                count = 0
                print("submit job reached the upper limit, waiting for some time...")
                sleep(7200)
