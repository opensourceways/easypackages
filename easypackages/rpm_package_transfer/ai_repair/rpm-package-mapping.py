...
def range_validator(min_value, max_value):
    def validator(value):
        ...
        if not min_value <= fvalue <= max_value:
            raise argparse.ArgumentTypeError(f"值必须在 {min_value} 和 {max_value} 之间")
        return fvalue
    return validator
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', type=str, required=True,
                        help="待匹配的包名字符串，格式为\"package_name1 package_name2\"，例如：\"389-ds-base 389-ds-base-devel\"")
    parser.add_argument('-b', type=range_validator(0, 1), required=True,
    ...
    res_str_similar_sort = dict(sorted(res_str_similar.items(), key=lambda item: item[1], reverse=True))
