...
#   RPMBUILD_INSTALL_FLAG: rpm包已安装标志
#----------------------------------------------------------------------------------
...
check()
{
    log_msg "source list file: ${LIST_PATH}"
...
do
    ...
    # 检查是否属于本list
    if ! grep -q "/${rpm_name_no_exten}" "${LIST_PATH}"; then
        log_msg "[warn] not belong to this batch: [${rpm_name}}"
        continue
    fi
